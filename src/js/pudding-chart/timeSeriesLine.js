  ...
  function createChart(el) {
    ...
    function mapToArray(map) {
      ...
      map.forEach(function (val, key) {
        res.push({
          year: key,
          womenPolarityMed: val[0],
          allPolarityMed: val[1],
        });
      });
      return res;
    }
    ...
    const Chart = {
      ...
      init() {
        ...
        x = d3.scaleTime().domain(d3.extent(data, (d) => d.year));
        ...
        $xAxis = $axis.append("g").attr("class", "stackedChartyTicks");

        $yAxis = $axis.append("g").attr("class", "lineChartyAxis");
        ...
        $womenAnno = $vis.append("text").attr("class", "polarityCompFemText");

        $allAnno = $vis.append("text").attr("class", "polarityCompAllText");
      },
      ...
      resize() {
        ...
        $yAxis
          ...
          .call(
            d3.axisLeft(y).tickSize(0).tickValues([0.1, 0.2, 0.3, 0.4, 0.45])
          )
          .call((g) => g.select(".domain").remove());

        $axisText.attr("x", -height / 1.1).attr("y", 10);

        $lineW.x((d) => x(d.year)).y((d) => y(d.womenPolarityMed));

        $lineA.x((d) => x(d.year)).y((d) => y(d.allPolarityMed));
        ...
        $allAnno
          .text("Headlines about other topics")
          .attr("opacity", 0)
          .attr("x", x(d3.max(data, (d) => d.year)))
          .attr("y", function (d) {
            let dataLength = data.length - 1;
