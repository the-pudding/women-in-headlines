...
function setupTimeSeriesLine(data) {
  chartTimeSeriesLine = $timeSeries.datum(data).puddingTimeSeriesLine();
}

/* BARBELL */
function setupLollipop(data) {
  chartLollipop = $lollipop.datum(data).puddingLollipop();
}

/* BUBBLE */
function setupBubbleP(data) {
  $bubble = d3.select(`#chartP`);
  chartBubbleP = $bubble.datum(data).puddingBubble();
}

function setupBubbleB(data) {
  $bubble = d3.select(`#chartB`);
  chartBubbleB = $bubble.datum(data).puddingBubble();
}

/* TEMPORAL LINE */
function setupTemporalLine(data) {
  chartTemporalLine = $temporalLine.datum(data).puddingTemporalLine();
}
...
function spanLeave() {
  ...
  if (
    span === "crimeandviolence" ||
    span === "femalestereotypes" ||
    span === "empowerment" ||
    span === "peopleandplaces" ||
    span === "raceethnicityandidentity"
  ) {
    ...
    d3.selectAll(`.stackedBars .raceethnicictyandidentity_class`).attr(
      "opacity",
      "1"
    );
  }
